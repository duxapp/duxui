...
export const Rect = /*@__PURE__*/ pure(() => {
  const Rect_ = () => {
    return null
  }

  Rect_.displayName = 'DuxSvgRect'

  Rect_.bbox = ({ x = 0, y = 0, width, height }) => {
    return {
      x,
      y,
      width,
  ...
  Rect_.range = (touch, { x = 0, y = 0, width, height }) => {
    return touch.x >= x && touch.x <= x + width
      && touch.y >= y && touch.y <= y + height
  }

  Rect_.draw = (ctx, { x = 0, y = 0, width, height, rx = 0, ry = 0, fill, stroke }) => {
